...
class WeekPatternController extends Controller
{
    ...
    /**
     * یک الگوی کاری اتمی (WorkPattern) را بر اساس مشخصات روز پیدا می‌کند یا می‌سازد.
     * این کار از ایجاد الگوهای تکراری (مثلاً ۱۰ الگوی ۸-۱۶) جلوگیری می‌کند.
     */
    public function findOrCreateAtomicPattern(array $dayRule): int
    {
        if (!$dayRule['is_working_day']) {
            $attributes = [
                'type' => 'fixed',
    ...
    /**
     * تبدیل عدد روز هفته به نام ستون در دیتابیس
     */
    private function getDayColumnName(int $dayOfWeek): string
    {
        $map = [
            0 => 'saturday_pattern_id',
            1 => 'sunday_pattern_id',
            2 => 'monday_pattern_id',
            3 => 'tuesday_pattern_id',
            4 => 'wednesday_pattern_id',
            5 => 'thursday_pattern_id',
            6 => 'friday_pattern_id',
        ];
    ...
    private function validateRequest(Request $request, $ignoreId = null)
    {
        ...
        return Validator::make($request->all(), [
            ...
            'days.*.work_duration_minutes' => 'required_if:days.*.is_working_day,true|nullable|integer|min:0|max:1440',
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($validatedData['days'] as $dayRule) {
                $patternId = $this->findOrCreateAtomicPattern($dayRule);

                $dayColumnName = $this->getDayColumnName($dayRule['day_of_week']);

                $dayPatternIds[$dayColumnName] = $patternId;
            }
     ...
     */
    public function show(WeekPattern $weekPattern)
    {
        $weekPattern->load([
            'saturdayPattern', 'sundayPattern', 'mondayPattern',
            'tuesdayPattern', 'wednesdayPattern', 'thursdayPattern', 'fridayPattern'
    ...
    public function update(Request $request, WeekPattern $weekPattern)
    {
        ...
        try {
            ...
            // منطق دقیقاً مشابه store است:
            // ۱. پترن‌های اتمی را پیدا یا ایجاد کن
            foreach ($validatedData['days'] as $dayRule) {
                $patternId = $this->findOrCreateAtomicPattern($dayRule);
                $dayColumnName = $this->getDayColumnName($dayRule['day_of_week']);
                $dayPatternIds[$dayColumnName] = $patternId;
            ...
            $weekPattern->update($weekPatternData);
     ...
     */
    public function destroy(WeekPattern $weekPattern)
    {
        if ($weekPattern->workGroups()->exists() || $weekPattern->employees()->exists())
        {
            return response()->json(['message' => 'Cannot delete this week pattern because it is assigned to one or more work groups or employees.'], 409);
        }
        $weekPattern->delete();
        PruneOrphanWorkPatterns::dispatch();
