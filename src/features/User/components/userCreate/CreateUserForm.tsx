...
const FormSectionCard = ({
    ...
    <div className={`bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm mb-6 relative overflow-hidden transition-all hover:shadow-md ${className || ''}`}>
...
export const CreateUserForm: React.FC<{ organizationId: number }> = ({ organizationId }) => {
    ...
    // ✅ اصلاح: استفاده از as const برای مقادیر literal تا تایپ‌اسکریپت خطا نگیرد
    const defaultValues: CreateUserFormData = useMemo(() => ({
        user_name: "",
        email: "",
        password: "",
        role: "user",
        status: "active" as const, // ✅ Fix TS2322
        employee: {
            ...
            nationality_code: "",
            father_name: "",
            birth_date: "", // Zod string expects string, initialized as empty
            address: "",
            position: "",
            starting_job: "",
            gender: "male" as const, // ✅ Fix TS Type
            is_married: false,
            education_level: "diploma" as const, // ✅ Fix TS Type
    ...
    // استفاده از FormProvider برای دسترسی کامپوننت‌های فرزند (مثل ImageUploader) به متدهای فرم
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <FormSectionCard title="اطلاعات حساب کاربری" icon={Lock} className="z-40">
                    ...
                    <Controller name="role" control={control} render={({ field }) => (
                        <SelectBox
                            label="نقش کاربری *"
                            placeholder="انتخاب نقش" // ✅ Fix TS2741
                    ...
                    <Controller name="status" control={control} render={({ field }) => (
                        <SelectBox
                            label="وضعیت *"
                            placeholder="انتخاب وضعیت" // ✅ Fix TS2741
                ...
                <FormSectionCard title="مشخصات فردی و تصاویر" icon={UserIcon} className="z-30">
                    ...
                    <Controller name="employee.gender" control={control} render={({ field }) => (
                        <SelectBox
                            label="جنسیت *"
                            placeholder="انتخاب جنسیت" // ✅ Fix TS2741
                    ...
                    <Controller name="employee.is_married" control={control} render={({ field }) => (
                        <SelectBox
                            label="وضعیت تاهل *"
                            placeholder="انتخاب وضعیت تاهل" // ✅ Fix TS2741
                    ...
                    <Controller name="employee.education_level" control={control} render={({ field }) => (
                        <SelectBox
                            label="تحصیلات *"
                            placeholder="انتخاب تحصیلات" // ✅ Fix TS2741
                            options={EDUCATION_OPTIONS}
                            value={EDUCATION_OPTIONS.find(o => o.id === field.value) || null}
                            onChange={(opt) => field.onChange(opt?.id)}
                ...
                <FormSectionCard title="اطلاعات سازمانی" icon={Building2} className="z-20">
                    ...
                    <Controller name="employee.work_group_id" control={control} render={({ field }) => (
                        <SelectBox
                            ...
                            placeholder="انتخاب کنید (اختیاری)" // ✅ Fix TS2741
                        />
                    )} />
                ...
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] flex justify-end gap-3 z-50 transition-transform duration-300">
